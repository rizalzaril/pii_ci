...
class Erp extends CI_Controller {
	...
	private function _call_write($obj, $id = 0, $data = array()) {
		$this->_call ( $obj, 'write', array(array (
				$id 
		), 
				$data 
		));
	...
	private function _call_write_ada($obj, $id = 0, $data = array()) {
		return $this->_call_ada ( $obj, 'write', array(array (
				$id 
		), 
				$data 
		));
	...
	public function reg_member(){
		...
		{			
			...
			$p = 444;
			if($totalX == '1650000') { $p = 445;$tag = $this->IP_2024;$name = 'Fullpayment IPM';}//IPM 
			else if($totalX == '2200000') { $p = 446;$tag = $this->IP_2024;$name = 'Fullpayment IPU';}//IPU 
			else if($tipe_faip == 'IPM') { $p = 472;$tag = $this->IP_2024;$name = 'IPM Pelunasan';}//IPM 
			else if($tipe_faip == 'IPU') { $p = 473;$tag = $this->IP_2024;$name = 'IPU Pelunasan';}//IPU 
		...
		{			
			...
			$tag = $this->IP_2024;//IPP
			$p = 490;
			if($tipe_faip == 'IPU') { $p = 491;$tag = $this->IP_2024;$name = 'IPU Perpanjang (Peningkatan)';}//IPM 
			else if($tipe_faip == 'IPM') { $p = 492;$tag = $this->IP_2024;$name = 'IPM Perpanjang (Peningkatan)';}//IPU 
