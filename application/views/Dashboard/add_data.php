...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		const inputKeterangan = document.getElementById('inputKeterangan');
		const inputGrade = document.getElementById('inputGrade');
>>>>>>> 39cdf7b446a976ca285f8e19d846d1ffc495c8b2

		inputGrade.addEventListener('input', function() {
			const value = inputGrade.value.trim();

			// Cek apakah input angka valid (gunakan match untuk angka desimal/positif saja)
			const isValidFormat = value.match(/^\d+(\.\d+)?$/);

			// Reset error state
			inputGrade.classList.remove('is-invalid');

			if (isValidFormat) {
				const grade = parseFloat(inputGrade.value.trim()); // HARUS ambil dari inputGrade
				const keterangan = parseFloat(inputKeterangan.value.trim()); // dari inputKeterangan

				const ket1 = keterangan >= 10 && keterangan < 15;
				const ket2 = keterangan >= 15 && keterangan < 20;

				if (ket1) {
					if (grade < keterangan) {
						console.log('⚠️ Error: grade tidak boleh kurang dari keterangan (10-14)');
					...
					if (grade > keterangan) {
						console.log('⚠️ Error: grade tidak boleh lebih dari keterangan (15-19)');
						inputGrade.classList.add('is-invalid');
					} else {
						console.log('✅ OK');
						inputGrade.classList.remove('is-invalid');
					...
					console.log('⚠️ Keterangan di luar rentang valid (10–20)');
					inputKeterangan.classList.add('is-invalid');
				}
			} else {
				alert('⚠️ Input grade harus berupa angka yang valid');
				inputGrade.classList.add('is-invalid');
			}
