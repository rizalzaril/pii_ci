...
EOD;
  ...
  function member_serti()
  {
    ...
    $id = $this->session->userdata('user_id');
    $cari = $this->simpan_model->cari_di_member($id);
    if ($cari != null) {
      $nokta = $cari->no_kta;
    }
    //--------------------------------------------------------------------------------------------

    $cari = $this->simpan_model->ambil_nama($nokta);
    if ($cari != null) {
      $namad = $cari->namad;
      $namab = $cari->namab;
      $nama = $namad . ' ' . $namab;
    ...
    //        $nokta = substr($nokta1,1,5) ; echo $nokta ; exit() ;
    //------------- APEC ----------------------				        
    $cari = $this->simpan_model->cari_data_apec($nokta1);
    if ($cari != null) {
      $ketemu_apec = 'y';
      $kode_apec = $cari->kode;
      $nosip = $cari->nosip;
      $noreg = $cari->reg_urut;
      $data['kode_apec'] = $kode_apec;
      $data['nosip']  = $nosip;
      $data['noreg']  = $noreg;
    } else {
      $ketemu_apec = 't';
    ...
    //------------------ ACPE ------------------				        
    $cari = $this->simpan_model->cari_data_acpe($nokta1);
    if ($cari != null) {
      $ketemu_acpe = 'y';
      $no_acpe = $cari->no_acpe;
      $doi = $cari->doi;
      $new_pe = $cari->new_pe_no;
      $bk_acpe = $cari->bk_acpe;
    } else {
      $ketemu_acpe = 't';
      $no_acpe = '';
      $doi = '';
    ...
    if ($ketemu_acpe == 'y') {
      $data['kode_acpe'] = $no_acpe;
      $data['doi']  = $doi;
      $data['new_pe']  = $new_pe;
      $data['bk_acpe']  = $bk_acpe;
    } else {
      $data['kode_acpe'] = $no_acpe;
      $data['doi']  = $doi;
      $data['new_pe']  = $new_pe;
    ...
    //--------------------- AER ------------------------			        				        
    $cari = $this->simpan_model->cari_data_aer($nokta1);
    if ($cari != null) {
      $ketemu_aer = 'y';
      $no_aer = $cari->no_aer;
      $grade = $cari->grade;
    ...
    //-------------- SKIP & STRI -----------------------------					
    $cari_cert = $this->simpan_model->cari_di_member_serti_akhir($nokta1);
    if ($cari_cert != null) {
      $ketemu_cert = 'y';
      $sk_skip = $cari_cert->skip_sk;
      $skip_from = $cari_cert->skip_from_date;
      $skip_thru = $cari_cert->skip_thru_date;
      $skip_id = $cari_cert->skip_id;
      $cert_type = $cari_cert->certificate_type;
      if ($cert_type == 1) {
        $cert_ket = 'IPP';
      }
      if ($cert_type == 2) {
      ...
      $stri_id = $cari_cert->stri_id;
      $stri_sk = $cari_cert->stri_sk;
      $stri_tipe = $cari_cert->stri_tipe;
      $stri_from = $cari_cert->stri_from_date;
      $stri_thru = $cari_cert->stri_thru_date;
    } else {
      $nokta = substr($nokta1, 1, 5);
      $cari_cert = $this->simpan_model->cari_di_member_serti_akhir($nokta);
      if ($cari_cert != null) {
        $ketemu_cert = 'y';
        $sk_skip = $cari_cert->skip_sk;
        $skip_from = $cari_cert->skip_from_date;
    ...
    $data['nokta1']    = $nokta1;
    $data['sk_skip']   = $sk_skip;
    $data['skip_from'] = $skip_from;
    $data['skip_thru'] = $skip_thru;
    $data['skip_id']   = $skip_id;
    $data['cert_type'] = $cert_type;
    $data['cert_ket']  = $cert_ket;

    $data['stri_id']   = $stri_id;
    $data['stri_sk']   = $stri_sk;
    $data['stri_tipe'] = $stri_tipe;
    $data['stri_from'] = $stri_from;
    $data['stri_thru'] = $stri_thru;
